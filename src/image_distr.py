...
# TODO: https://docs.opencv.org/3.0-beta/doc/py_tutorials/py_imgproc/py_morphological_ops/py_morphological_ops.html
# TODO: add more & add contrast
class DistortImages:
    def __init__(self):
        self.__path = None
        self.images = None
    ...
    def compression(self, file_name, save_path, quality_values):
        ...
        for quality_val in quality_values:
            new_name = file_name[:-4] + '_' + "compression{:03d}".format(quality_val) + file_name[-4:]
    ...
    def gaussian_noise(file_name, img, save_path, sigma_values, mu=0):
        for sigma_value in sigma_values:
            new_name = file_name[:-4] + '_' + "gaussian_noise{:03d}".format(sigma_value) + file_name[-4:]
